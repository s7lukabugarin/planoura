...
const WorkoutEditModal: React.FC<WorkoutEditModalProps> = ({
    ...
    if (fetchedClass) {
      ...
      try {
        const dataToSend = {
          ...
          exercise_ids: selectedExercises?.map((e: any) => e.id),
          member_ids:
        ...
        const updatedClass = await updateClass(fetchedClass.id, dataToSend);
  ...
  useEffect(() => {
    setSelectedExercises(fetchedClass?.exercises);
    setWorkoutTitle(fetchedClass?.name);
    setWorkoutDescription(fetchedClass?.description);
